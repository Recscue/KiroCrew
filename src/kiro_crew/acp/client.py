...
class AcpClient:
    ...
    def _verify_opencode_routing(self, argv: list[str], config_content: str) -> str:
        ...
        Returns ``""`` when the required value is in force, else the reason, in the
        vocabulary :func:`acp_tool_gate.seeded_setting_issue` owns.
        ...
        *argv* arrives ALREADY SANDBOX-WRAPPED, and that is a security property
        ...
        # The SAME scrub the spawn below applies, and for the same reason: this is a
        # foreign harness binary, and the gateway's own environment carries channel
        # tokens, cloud secrets and an agent socket that no harness may see. The
        # read-back runs BEFORE the spawn, so inheriting ``os.environ`` verbatim
        # would hand a child every one of them a few lines ahead of the code that
        # strips them.
        ...
        try:
            ...
            return f"the resolved configuration could not be read back ({exc})"
        ...
        if not isinstance(resolved, dict):
            return "the resolved configuration could not be parsed"
        observed = _opencode_uniform_permission(resolved.get(setting_key))
        if isinstance(observed, str):
            # The value came out of the operator's own config and travels into a
            # refusal that reaches the dashboard and the chat card, so it goes
            # through the same two scrubs every other backend-sourced string in this
            # module does. A rule map can name anything -- a URL, a token-shaped
            # literal -- and a refusal is not a reason to publish it.
            observed, _ = redact_exfiltration_urls(observed)
            observed, _ = redact_credentials(observed)
        return acp_tool_gate.seeded_setting_issue(self.backend, observed)
    ...
    async def _spawn(self) -> None:
        ...
        if self._is_claude:
            ...
            try:
                routing_issue = await asyncio.to_thread(
    ...
    async def _initialize_session(self) -> None:
        ...
        if resume_sid and self._can_load_session:
            ...
            if self._is_claude:
                ...
                # Same shape as claude, and for the same reason: the harness keeps
                ...
                # here would make file_ok always False — such a session could
                # never resume, it would silently start fresh every time — which is
                # why the _meta block below gives neither a session_file.
                #
                # opencode belongs here on its own evidence: its ``initialize``
                # result advertises ``loadSession: true`` and its session ids are its
                # own (``ses_…``), so a resume it could serve would otherwise be
                # dropped by a file check against a kiro path that never exists.
                session_file = ""
                file_ok = True
            else:
            ...
            if file_ok:
                try:
                    ...
                    if self._is_claude:
                        ...
                        # Neither carries a Crew-side session file nor reads any
                        # _meta of ours, so each gets neither key rather than the
