...
def is_shell_call(inp: _Input) -> bool:
    ...
    if not kind_unknown or shell_command_of(inp) is None:
        return False
    return title in SHELL_STUB_TITLES or (
        title.startswith("Running:") and not title.startswith("Running: @")
    )
    ...
    *,
    ...
    return _classify(
        _Input(
            ...
            mcp_tool_title=mcp_tool_title or "",
            cwd=cwd or "",
        )
    )
...
def render_tool_action(action: ToolAction) -> str:
    ...
    if t == "github":
        if action.get("target"):
            return f"GitHub {action['noun']} {action['verb']} {action['target']}"
        return f"GitHub {action['noun']} {action['verb']}"
    if t == "github_api":
    ...
    if t == "mcp":
        ...
        return f"{label}: {action['arg']}" if action.get("arg") else str(label)
