...
- ``pr_checks`` that comes back PENDING on a DRAFT pull request is ``refused``,
  not retried: where a repo has an aggregate check gated on the draft flag, that
  wait is an infinite loop rather than a wait. A draft whose checks resolve is
  judged on them like any other PR.
...
def _evaluate(item):
    ...
    if kind == "pr_checks":
        ...
        #
        # A draft PR is not unconditionally unsatisfiable: where a repo has no
        # aggregate check gated on the draft flag, a draft's checks resolve and
        # `gh pr checks` exits 0 - a `pass` this script must keep returning,
        # since this skill ships to arbitrary repos. What IS unsatisfiable is
        # the combination: pending, and pending BECAUSE the PR is a draft, which
        # no amount of further polling changes. Asking only about a pending run
        # also costs nothing on the pass and fail paths, which is every cycle
        # that was never going to loop.
        if verdict == "pending" and _pr_is_draft(pr, repo):
            return (
                "refused",
                f"PR #{pr} is a draft and its checks are still pending; a "
                "readiness check gated on the draft flag will not resolve on "
                "its own -- mark it ready for review or change the acceptance "
                "kind",
            )
