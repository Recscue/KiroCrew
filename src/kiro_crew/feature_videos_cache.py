...
class FeatureVideoCache:
    ...
    def _evict_now(self, release: str, dashboard: object) -> None:
        ...
        evict(release, max_bytes=int(max_mb * 1024 * 1024), keep_releases=keep)
...
def resolve_served_path(release: str, name: str) -> "Path | None":
    ...
    The containment re-check after ``resolve()`` is not redundant with the name
    validation: it is what still holds if a symlink inside the cache directory
    points out of it.
    """
    ...
    try:
        ...
        return None
    try:
        resolved = (folder / name).resolve(strict=True)
