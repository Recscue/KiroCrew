...
def _offer_pools(running_version: str) -> tuple[tuple[Offer, ...], tuple[Offer, ...]]:
    ...
    if current is None:
        ...
        return pool, ()
    # The CACHED answer: /next is polled, and an audited evaluation per request
    # would write a governance_decision SEL row per poll. The transfer itself
    # re-checks the audited answer before every outbound request, so a denial can
    # be at most one TTL stale here and cannot cost more than one offer.
    allow_remote = manifest_mod.download_permitted_cached()
...
def select_next(running_version: str) -> "Offer | None":
    ...
    """
    preferred, fallback = _offer_pools(running_version)
    pool = preferred or fallback
    return _rng.choice(list(pool)) if pool else None
...
async def api_feature_videos_next(request: web.Request) -> web.Response:
    ...
    where the offer carries ``source`` (``local`` or ``remote``) and the ``src``
    for that source — resolved here so a client never has to guess whether a clip
    is on disk.
