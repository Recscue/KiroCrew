...
def _audit_unevaluable() -> None:
    ...
    try:
        # Local import is DELIBERATE (matches the other SEL sites): the test
        # suite patches ``kiro_crew.sel.sel``, which a load-time binding bypasses.
...
def _fetch_json(url: str) -> "dict | None":
    ...
    except (urllib.error.URLError, OSError, TimeoutError, ValueError, RecursionError) as exc:
        # RecursionError alongside ValueError: json.loads raises it on a document
        # nested past the interpreter's limit, and it is neither an OSError nor a
        # ValueError — without it a hostile response would propagate out of a
        # background task instead of reading as "no manifest".
        logger.info(
