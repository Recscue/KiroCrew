...
async def api_update_arm(request: web.Request) -> web.Response:
    ...
    #
    # Known limit, accepted: `_do_update_check` is single-flight (`_check_in_flight`),
    # so a status poll's check running at this moment makes the re-check a no-op and
    # this arms the cached verdict silently. Same shape as the channel-switch
    # handler's caveat, and it arms nothing worse than a caller that did not
    # re-check at all — which is why closing it is a follow-up. The Electron twin
    # IS closed (`checkInFlight` in auto-update.js lets the click await the running
    # check's answer); the difference is that this module keeps no handle on the
    # in-flight check to await.
