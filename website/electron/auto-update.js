...
function initAutoUpdate(deps) {
  ...
  function deferredInstallOnQuit(event) {
    // `installing` and `verifyingStage` stand for "the manual path already owns
    // this install", and both windows end in a quit of their own — so standing
    // down here means letting that quit proceed, not cancelling anything. Without
    // them a Cmd+Q lands a SECOND quitAndInstall() against the same install
    // directory, plus a second forceExitFailsafe: `installing` is already claimed
    // while applyUpdateAndRestart awaits stopGateway(), and the `checking` abort
    // below cannot see it (that check settled before the dispatch, or was
    // abandoned and is deliberately exempt). No notifyInstallCanceled on this
    // return, unlike the two below it: the install IS proceeding.
    if (quitHandled || installing || verifyingStage || !updateReady) return;
    ...
    (async () => {
      ...
      if (!updateReady || (checking && !abandonedCheck)) {
        log.info("[update] stage invalidated or a check straddles the quit — quitting without installing");
