...
test("a FAILED deferred install finishes the quit instead of surviving with a dead dashboard", async () => {
  ...
  // Neither claim may latch: the stage is still on disk, so a click has to be able
  // to dispatch it — and if THAT one fails it is a manual install, which recovers
  // in place. A `deferredInstalling` left standing would quit the app on a failure
  // the user is sitting in front of, which is the same "app leaves without
  // explanation" bug in the opposite direction.
  h.feedServes("1.1.0");
  await u.install();
  assert.strictEqual(h.calls.quitAndInstall.length, 2, "the staged build must still be installable");
  ...
  assert.strictEqual(h.calls.installFailures, 1, "a manual install's failure must bring the gateway back");
  assert.strictEqual(h.calls.quits, 1, "and must not quit the app: only the earlier deferred one may");
});
...
test("a quit during a manual install does not hand the installer a second dispatch", async () => {
  ...
  assert.strictEqual(
    prevented.value,
    false,
    "and must not prevent the quit either: the install already underway ends in its own quit",
  );
