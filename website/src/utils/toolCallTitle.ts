 ...
 *
 * The split is load-bearing: `src/kiro_crew/acp/tool_call_title.py` is a
...
export { classifyShellCommand, shortDisplayPath, summarizeSegment, tokenizeShell } from './shellCommandParse'
...
export type ToolCallTitleInput = {
  ...
  mcpServer?: string
  /** The server-declared `Tool.title` display name, when known. */
  mcpToolTitle?: string
...
function classifyMcp(inp: ToolCallTitleInput): ClassifiedToolCall | null {
  ...
  const label = str(inp.mcpToolTitle)
  return {
    action: { type: 'mcp', tool, ...(label ? { title: label } : {}), ...(arg ? { arg } : {}) },
    more: 0,
...
function isShellCall(inp: ToolCallTitleInput): boolean {
  ...
  if (!kindUnknown || shellCommandOf(inp) === undefined) return false
  return SHELL_STUB_TITLE_RE.test(title) || (title.startsWith('Running:') && !title.startsWith('Running: @'))
}
  ...
  switch (action.type) {
    ...
    case 'github':
      return action.target
        ? i18nT('utils.toolCallTitle.github_target', { noun: action.noun, verb: action.verb, target: action.target })
        : i18nT('utils.toolCallTitle.github', { noun: action.noun, verb: action.verb })
    case 'github_api':
