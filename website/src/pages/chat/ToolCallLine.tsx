...
export default memo(function ToolCallLine({ message, running: _running, slot, onFileOpen, disclosure, disclosureKey, onDisclosureChange, appInPanel, onOpenApp, transcriptHot = false }: { message: ChatMessage; running: boolean; slot?: string; onFileOpen?: (path: string) => void; disclosure?: boolean; disclosureKey?: string; onDisclosureChange?: (key: string, expanded: boolean) => void; appInPanel?: boolean; onOpenApp?: (toolCallId: string) => void;
  ...
  const derived = useMemo(() => deriveToolCallTitle({
    title: label,
    kind: toolKind,
    rawInput: input,
    isShell,
    ...
    mcpServer,
    cwd: projectDir,
  }), [label, toolKind, input, isShell, trustedToolName, mcpServer, projectDir])
  ...
  // A purpose-less shell call's label is the derived title (or the raw command,
  // cut to ~80 chars, when the command did not parse). The collapsed row's
  // `truncate` (LABEL_COLLAPSED_CLASS) bounds how much of a long PURPOSE is
  // visible; a flood-length label in simplified mode is substituted with a
  // derived command digest (binaries + redirect target) so the visible line is
  // meaningful. Short labels pass through untouched.
