...
export default function MembersPage() {
  ...
  // A failed roster read is said on the roster (the notice below the header),
  // and an EMPTY roster's only content is its "no one yet" copy and the
  // create button; a rail has room for neither, so the fold yields to both.
  const rosterEmpty = loaded && !loadError && members.length === 0
  const railed = rosterIsRail({ winW, isMobile, pref: rosterCollapsedPref }) && !loadError && !rosterEmpty
