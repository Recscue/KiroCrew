...
describe('ConnectionsPanel — one card per pre-registered provider', () => {
  it('renders a card per client with its configured state and settings anchor', async () => {
    ...
    expect(within(gh).getByText('Not configured')).toBeInTheDocument()
    ...
    expect(within(as).queryByText('Not configured')).not.toBeInTheDocument()
  ...
  it('links each card to its registration runbook', async () => {
    await mount()

    const link = within(cardFor('github')).getByRole('link', { name: /Open the GitHub registration runbook/ })
...
describe('ConnectionsPanel — removing a client', () => {
  it('calls the delete endpoint for the provider and re-reads the list', async () => {
