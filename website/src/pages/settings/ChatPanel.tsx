...
export function ChatPanel() {
  ...
  const fvQ = useQuery<FeatureVideoStatus>({
    queryKey: ['featureVideoStatus'],
    queryFn: () => api.featureVideoStatus(),
  ...
  const fvFetchMut = useMutation({
    mutationFn: () => api.featureVideoFetchAll(),
   ...
   *
   * Returns null while the read is still out or the feature is off, so the row is
   * absent rather than showing a zero-of-zero that reads like an empty cache.
   */
