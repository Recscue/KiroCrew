...
describe('WebPreviewPanel — address bar launcher (non-native transport)', () => {
  ...
  it('maps the gateway\'s own invalid_url refusal to the same card', async () => {
    // A caller that reaches the gateway with a refused shape anyway (an older
    // panel, a hand-made request) gets the same instruction, not the raw text.
    ...
    expect(card.textContent).toMatch(/frame's own address bar/)
    expect(screen.queryByTestId('web-preview-launch-refused')).toBeNull()
    expect(screen.queryByText('Try again')).toBeNull()
  })
