...
describe('StartupVideoModal — the reachability probe', () => {
  it('asks the SERVER once, by clip id, and opens on ok', async () => {
    await mount()
    await waitFor(() => expect(dialog()).toBeInTheDocument())
    expect(mockedApi.featureVideoProbe).toHaveBeenCalledTimes(1)
    expect(mockedApi.featureVideoProbe).toHaveBeenCalledWith(clip.id, undefined)
  ...
  it('never probes the asset from the page', async () => {
    ...
    // network failure for a perfectly healthy file -- and a probe that is wrong for
    // half the catalog is worse than none.
    await mount()
    await waitFor(() => expect(dialog()).toBeInTheDocument())
  ...
  it('on a probe that cannot be asked: same outcome, reason in `detail`', async () => {
    // An older gateway without the route, or the network. An unanswerable question
    // is not permission to open a player.
    __resetErrorJournalForTests()
    mockedApi.featureVideoProbe.mockRejectedValue(new Error('HTTP 404'))
    const { onClose } = await mount()
  ...
  it('carries the active slot key on the probe too', async () => {
    ...
    await waitFor(() => expect(dialog()).toBeInTheDocument())
    expect(mockedApi.featureVideoProbe).toHaveBeenCalledWith(clip.id, 'dashboard:slot-9')
  })
})
...
describe('StartupVideoModal — a clip that streams from the CDN', () => {
  ...
  it('preloads the METADATA so the controls work, and still never autoplays', async () => {
    // Without the header a streamed clip shows no duration and its seek bar cannot
    // be dragged -- working controls for a few kilobytes. The bytes of the clip
    // itself still wait for a press.
    await mount()
    await waitFor(() => expect(video()).toBeInTheDocument())
