 ...
 *                     says the bytes come over the network, and a player that
 *                     preloads its metadata so the controls work. Asserted, since
 *                     "which preload" is a claim about the DOM, not a look.
...
/* ── 04 a clip that streams from the CDN ─────────────────────────────────── */
{
  ...
  // Metadata, not the clip: without the header the controls show no duration and
  // the seek bar cannot be dragged.
  const preload = await page.locator('[data-testid="startup-video"]').getAttribute('preload')
  if (preload !== 'metadata') fail(`remote clip preloads "${preload}" — expected "metadata"`)
