...
class TestDraftPullRequest:
    ...
    def test_a_pending_draft_is_refused(self, monkeypatch):
        ...
        assert "still pending" in evidence
    ...
    def test_the_evidence_does_not_claim_a_universal_gh_behavior(self):
        """The refusal names THIS repo's cause, not a property of ``gh``.

        "a draft can never pass" is a claim about a repo whose aggregate check
        gates on the draft flag, and this skill ships to repos that do not. The
        evidence a human reads must not assert the general version.
        """
        src = SCRIPT.read_text(encoding="utf-8")
        assert "cannot pass while draft" not in src
        assert "will not resolve on" in src
