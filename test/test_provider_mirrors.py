...
class TestCodexMirror:
    ...
    def test_both_open_gaps_are_addressed_rather_than_decisions(self):
        """The two things codex CAN do that this transport cannot carry.

        Recorded as ``no-channel`` with a destination each, because an unaddressed
        gap is indistinguishable from a decision -- the documented cause of the
        hooks regression (``UNSUPPORTED_SPEC_KEYS`` in ``acp/kas_agents.py``).
        """
        rulings = mirror_for("codex").rulings()
        gaps = {c.value for c, r in rulings.items() if r.disposition is Disposition.NO_CHANNEL}
        assert gaps == {"hooks", "disabledTools"}
