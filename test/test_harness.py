...
class TestTeardownDoesNotTrustTheRootsExit:
    ...
    def test_the_posix_group_kill_still_runs_for_an_exited_root(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            h,
            "terminate_pgid",
            lambda pid, grace=None, pgid=None, wait=None: (killed.append(pid), True)[1],
        )
    ...
    def test_the_group_is_named_explicitly_so_a_reaped_leader_cannot_hide_it(
        self, monkeypatch
    ) -> None:
        """The group id is passed, not re-derived from a leader that may be gone."""
        from kiro_crew.testing import harness as h
        ...
        h._terminate_process_group(self._ExitedProc())
