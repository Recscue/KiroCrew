...
| 6 install probe | Done — `_probe_opencode` names `opencode` and the command that installs it. One component, and here that is not a simplification: the thing that would be missing is the thing that serves ACP. No `restart_required`, because the spawn path publishes no negative cache for the probe to read, and inventing that answer would be worse than omitting it. |
...
| 8 live spill | Reached — a live turn, and a frame corpus that is live for six of the seven required classes. |
