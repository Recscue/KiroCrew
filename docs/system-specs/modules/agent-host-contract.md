...
| Directive marker readable in the result | yes | **no**, and it does not need to be | yes | unmeasured, and it does not need to be | unverified — the captured turn's tool call was rejected by the harness's own argument schema before it ran, so no result text has been observed end to end |
...
| Wire title of an MCP tool call | `_meta.kiro` identity, title not needed | `Running: @<serverName>/<toolName>` (measured; the `Running: ` prefix is the backend's, and `directive_tool_from_call` strips one) | raw Claude tool name `mcp__<server>__<tool>` | unmeasured — a codex MCP call whose title matches neither spelling above resolves to no directive tool and records no digest; declaring the spelling is part of onboarding the backend | unknown to this repository — no Crew MCP server can be mounted on this harness today, so no such call has been seen |
