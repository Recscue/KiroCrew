...
| Model picker order | User-authored display order for the model-selection dropdown, honored by every picker (chat composer, agents pages, sidebar bulk switcher, app pickers, cron job form); Auto stays pinned first, unlisted models append in backend order | Settings → Chat → **Model order** — drag to reorder, Reset to default order | `pages/settings/ModelOrderCard.tsx`, `providers/modelList.ts` (`applyModelOrder`), `hooks/useAvailableModels.ts` | `handlers/core.py` (`api_kirocrew_config_patch`, `str_list`) | `GET,PATCH /api/config/kirocrew` (`agent.model_order`) |
...
| `chat` | Chat behavior preferences, plain vs highlighted diffs, model picker order | `ChatPanel.tsx`, `ModelOrderCard.tsx` | `handlers/core.py` | `GET,PUT,PATCH /api/config/kirocrew` |
