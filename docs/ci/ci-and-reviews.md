...
description in the web UI yields the same URL). The lane downloads those URLs from the
body -- a committed image is still accepted -- reads each one and grounds visual
...
the runner (the job's egress allowlist already admits `github.com` and
`objects.githubusercontent.com`, the two hosts a download touches), so the reviewer
