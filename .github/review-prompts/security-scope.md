...
══════════════════════════════════════════════════════════════════
The classifier adjudicates `shell`, `flow`, and `cron` operations through the
...
Likewise, per platform: say for EACH of posix and windows whether a verdict was
obtained. A platform with no verdict is an open question, not a pass.
...
summary. The review is these sections and nothing else:
    ...
    PLATFORMS: posix <adjudicated | no verdict: why> | windows <adjudicated |
    no verdict: why>
...
- `CONCERNS` is the verdict for an unadjudicated guard, a missing platform
  verdict, or a narrowing you can argue but not confirm.
- `PASS` requires the tightening to have been adjudicated, on every platform the
  diff touches, with zero confirmed regressions.
